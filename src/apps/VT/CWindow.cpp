...
auto CWindow::InitializeVolumePkg(const std::string& nVpkgPath) -> bool
{
    ...
    try {
        ...
        std::cerr << "Failed to initialize volpkg: " << e.what() << std::endl;;
    }

    if (fVpkg == nullptr) {
        std::cerr << "Cannot open .volpkg: " << nVpkgPath << std::endl;;
        QMessageBox::warning(
            this, "Error",
...
void CWindow::OpenVolume(const QString& path)
{
    ...
    if (aVpkgPath.isEmpty()) {
        ...
        if (aVpkgPath.length() == 0) {
            std::cout << "Open .volpkg canceled" << std::endl;;
            return;
        }
    }
    ...
    if (extension != ".volpkg") {
        ...
        std::cerr << "Selected file is not .volpkg: " << aVpkgPath.toStdString() << std::endl;
