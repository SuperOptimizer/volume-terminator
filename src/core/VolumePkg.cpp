...
VolumePkg::VolumePkg(const std::filesystem::path& fileLocation) : _rootdir{fileLocation}
{
    for (const auto& d : ::ReqDirs(_rootdir)) {
        if (not std::filesystem::exists(d)) {
            std::cerr << "ERROR: VolumePkg directory missing " << d.filename().string() << std::endl;
            abort();
        }
        if (d != _rootdir) {
            ::keep(d);
...
void VolumePkg::loadSegmentationsFromDirectory(const std::string& dirName)
{
    ...
    if (!std::filesystem::exists(segDir)) {
        std::cout << "Segmentation directory '{}' does not exist " << dirName << std::endl;
        return;
    }
...
void VolumePkg::refreshSegmentations()
{
    ...
    if (!std::filesystem::exists(segDir)) {
        std::cout << "Segmentation directory " << _currentSegmentationDir << "  does not exist" << std::endl;
        return;
    }
    ...
    for (const auto& id : toRemove) {
        std::cout << "Removing segmentation " << id <<  " - no longer exists on disk" << std::endl;
    ...
    for (const auto& diskPath : diskPaths) {
        ...
        if (!found) {
            try {
                ...
                std::cout << "Added new segmentation " << s->id() << std::endl;
            }
            catch (const std::exception &exc) {
                std::cout << "Failed to load segment dir: " << diskPath.string() << " " << exc.what();
            }
        }
    }
}
